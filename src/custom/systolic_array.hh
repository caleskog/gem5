...
private:
  // void readMemoryDouble(ThreadContext *tc, Addr virt_addr, double *buffer,
  //                       size_t count);
  ...
  // template <class T> T getVal(ExecContext *xc, Addr vaddr, uint64_t index);
  // template <class T>
  // void setVal(ExecContext *xc, Addr vaddr, uint64_t index, T &val);
  template <class T>
...
T SystolicArray::getVal(ExecContext *xc, Addr vaddr, uint64_t index) {
  T val;
  Fault fault = gem5::initiateMemRead(xc, nullptr, vaddr + (index * sizeof(T)),
                                      val, Request::UNCACHEABLE);
  if (fault) {
    panic("[SystolicArray::getVal] initiateMemRead on vaddr %p failed!", vaddr);
  }
  return val;
};
template <class T>
void SystolicArray::setVal(ExecContext *xc, Addr vaddr, uint64_t index, T &val){
}; */
template <class T>
