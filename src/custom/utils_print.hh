...
template <typename T> static std::string toBinary(T n) {
  ...
  // Adding spaces between every 4 bits and double spaces between every 8
  // bits.
  int offset = 0;
  int size = r.length();
...
template <typename T> struct num_chars_impl {
  ...
  static size_t num_chars(uint32_t n) { return num_chars((uint64_t)n); }
  static size_t num_chars(uint16_t n) { return num_chars((uint64_t)n); }
  static size_t num_chars(uint8_t n) { return num_chars((uint64_t)n); }
  ...
  static size_t num_chars(int32_t n) { return num_chars((int64_t)n); }
  static size_t num_chars(int16_t n) { return num_chars((int64_t)n); }
  static size_t num_chars(int8_t n) { return num_chars((int64_t)n); }
  ...
  static size_t num_chars(double f) {
    ...
    sprintf(str, fmt, f);
    std::string s = str;
    // printf("%s, len=%ld, percision=%d\n", str, s.length(), precision);
    return s.length();
  }
  // For any string type
  static size_t num_chars(std::string s) { return s.length(); }
  static size_t num_chars(char *s) { return num_chars(std::string(s)); }
  static size_t num_chars(const char *s) { return num_chars(std::string(s)); }
};
...
/* template <typename T>
static void printImpl(const char *color, const int max_padding,
                      const ALIGNMENT padding_type, const Tensor3D<T> &tensor) {
    char sep = '[';
    for (const auto &inner_tensor : tensor) {
...
/**
 * @brief Retrive the max number of characters per column in a 2D list of Ts.
 */
template <typename T>
static size_t *getMaxPadding(T *list, int width, int height) {
  ...
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      padding = detail::num_chars_impl<T>::num_chars(list[j * width + i]);
...
// Check if fold expressions are supported
  ...
  int dummy[] = {0, ((void)detail::printImpl(color, max_padding, padding_type,
                                             std::forward<Args>(args)),
                     0)...};
  (void)dummy;
#endif
...
#define du_pprintc_cond(color1, color2, cond, ...)                             \
  if (cond)                                                                    \
    myutils::printing::printn(color1, 0, myutils::printing::ALIGNMENT::LEFT,   \
                              __VA_ARGS__);                                    \
  else                                                                         \
    myutils::printing::printn(color2, 0, myutils::printing::ALIGNMENT::LEFT,   \
                              __VA_ARGS__);
/**
 * @brief Macro for general color printing. It WILL print a new line.
 */
...
#define du_pprintc_cond_n(color1, color2, cond, ...)                           \
  if (cond)                                                                    \
    myutils::printing::print(color1, 0, myutils::printing::ALIGNMENT::LEFT,    \
                             __VA_ARGS__);                                     \
  else                                                                         \
    myutils::printing::print(color2, 0, myutils::printing::ALIGNMENT::LEFT,    \
                             __VA_ARGS__);
