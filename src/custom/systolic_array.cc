...
#endif
#ifdef KERNEL_DIM
  macro_str += myutils::string_format("KERNEL_DIM=(%d;%d;%d), ", KERNEL_DIM,
                                      MAX_COLS, MAX_ROWS);
#else
...
bool SystolicArray::load(u64 tid, u64 idx, Addr input) {
  DPRINTF(SA, "calling sa->load(tid:%ld, idx:%ld, input:%ld)\n", tid, idx,
          input);
  check_buffer_usage();
  ...
  PRINT_ARGUMENTS_LOAD2(tc, input, buf, SIZE, idx, tid);

  if (!getVals(tc, input, buf, SIZE)) {
    panic("[queue2] Cannot get values from vsrc %p address!", input);
  }

  c64 elm;
  int mask = 1;
  for (size_t i = 0; i < SIZE; i++) {
    tiles[tid]->weights[idx + i] = buf[i];
    elm.f = buf[i];
    mask &= elm.u;
  }
  if (mask != 0) {
...
void SystolicArray::queue2(ExecContext *xc, Addr vdst, Addr vsrc, u64 col) {
  ...
  */
  ...
  // implements initiateAcc and completeAcc. Though, gem5::readMemAtomicLE
  // should not need it, I think. But it crashed due to memRead not being
  // implemented, which I assume is due to the lack of the correct
  // template used, i.e. implements Execute, InitiateAcc, CompleteAcc.
...
Addr SystolicArray::queue(Addr vdst, u64 col, Addr vsrc) {
  ...
  int row_idx;
  for (size_t i = 0; i < SIZE; i++) {
    row_idx = KERNEL_DIM - 1 - (col + SIZE + i);
    col_idx = col + SIZE + i;
    outbuf[i] =
        mem2d(tiles[TID]->outWaitingMemory, KERNEL_DIM, row_idx, col_idx);
  }
...
Addr SystolicArray::process(Addr vdst, Addr vsrc) {
  ...
  for (size_t i = 0; i < SIZE; i++) {
    ...
    mem2d(tiles[TID]->inWaitingMemory, KERNEL_DIM, row_index, col_index) =
        inbuf[i];
  }
  //////////////////////////////////////////////////////////
  ...
  for (int i = 0; i < KERNEL_DIM; i++) {
    mem2d(tiles[TID]->inputMemory, KERNEL_DIM, i, 0) =
        mem2d(tiles[TID]->inWaitingMemory, KERNEL_DIM, i, KERNEL_DIM - 1);
    for (int j = KERNEL_DIM - 1; j > 0;
         j--) { // TODO: shift only the right-hand triangle
  ...
  for (int j = 0; j < KERNEL_DIM; j++) {
    for (int i = KERNEL_DIM - 1; i > 0;
         i--) { // TODO: shift only the right-hand triangle
      mem2d(tiles[TID]->outWaitingMemory, KERNEL_DIM, i, j) =
          mem2d(tiles[TID]->outWaitingMemory, KERNEL_DIM, i - 1, j);
    }
    //        std::cout << std::hex << mem2d(outputMemory, KERNEL_DIM,
    //        KERNEL_DIM, j) << std::endl;
...
void SystolicArray::init() {
  ...
  // (buffer[internal_buffer_size-1]) to not zero to indicate usage of the
  // buffer.
...
void SystolicArray::allocateOutputBuffer() {
  ...
  inform("Allocating output buffer of size %ld to address %#x", alloc_size,
         buffer_addr);
...
void SystolicArray::check_buffer_usage() {
  if (!checked_buffer_usage) {
    ...
    if ((uint64_t)vals[internal_buffer_size - 1] != 0) {
      ...
      inform("Using buffer_addr in custom instructions with buffer_size as %d",
             buffer_size);
    }
  }
}
